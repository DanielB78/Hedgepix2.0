...
import { runNportUpdate } from "./sec/nport.js";
...
async function main(): Promise<void> {
  ...
  console.log(`backfill-nport: fromYear=${args.fromYear} force=${args.force} dryRun=${args.dryRun}`);
  const supabase = createSupabase(loadConfig());
  const result = await runNportUpdate(supabase, args);
