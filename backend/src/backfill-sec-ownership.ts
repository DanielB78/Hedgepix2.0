...
import { runOwnership13Update } from "./sec/ownership13D13G.js";
...
async function main(): Promise<void> {
  ...
  console.log(
    `backfill-sec-ownership: fromYear=${args.fromYear} force=${args.force} dryRun=${args.dryRun}`,
  );
  const supabase = createSupabase(loadConfig());
  const result = await runOwnership13Update(supabase, args);
